...
langr = logging.getLogger("angr.manager")
#langr.setLevel(logging.DEBUG)
...
# EXPOSED
    ...
    def next_branch(self):
        ...
        while len(self.simgr.active) == 1:
            ...
            print
            l.debug("current: %#x %d %#x", current.addr,  self.bb_cnt, self.trace[self.bb_cnt])
            #self.check_stack(current)
            ...
            #succ = self.simgr.step(size=bbl_max_bytes)
            succ = self.simgr.step()
            l.debug("successors: %s | %s", str(self.simgr.active),str(self.simgr.unsat))
    ...
    def run(self, constrained_addrs=None):
        ...
        while (branches is None or len(branches.active)) and self.bb_cnt < len(self.trace):
            ...
            l.debug("branches: %s", str(branches))
...
# SYMBOLIC TRACING
    ...
    def check_stack(self, state):
        ...
        if state.memory.load(state.ip, state.ip.length).symbolic:
            ...
            print "found crash state", self.crash_state
	...
	#options.add(so.STRICT_PAGE_ACCESS)
        if self.crash_mode:
        ...
        main_addr = find_main(self.binary, project.entry)
        ...
        self.trace = self.trace[self.trace.index(main_addr):]

        if pg.active[0].addr != self.trace[0]:
            l.debug("START %#x, trace %#x, %s", pg.active[0].addr, self.trace[0], str(pg))
            pg = pg.explore(find=main_addr)
	...
	print "crash_winup", state, constrained_addrs
